...
export class SymmEasyEncrypt {
  constructor(password, format = "binary", encRounds = 8192) {
    this.pw = format == "text" ? Buffer.from(password, "base64") : password;
    this.rounds = encRounds;

    this.salt = crypto.randomBytes(16);
    this.key = crypto.pbkdf2Sync(this.pw, this.salt, this.rounds, 256 / 8, "sha256");
  }
  ...
  decrypt(msgRaw, format = "binary") {
    ...
    const key = crypto.pbkdf2Sync(this.pw, salt, this.rounds, 256 / 8, "sha256");
